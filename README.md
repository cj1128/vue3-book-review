...
- [Chapter 16: 解析器](#chapter-16-解析器)
- [Chapter 17: 编译优化](#chapter-17-编译优化)
- [Chapter 18: 同构渲染](#chapter-18-同构渲染)
...
- 文本模式
  - RCDATA, CDATA, RAWTEXT, DATA 等
  - 不同的模式下，解析器对文本的解析行为会有所不同
- HTML 实体解析
  - 命名字符引用，例如 `&lt;`
    - 因为历史原因，可能不存在末尾的分号
    - 存在分号时，执行完整匹配
...
- CSR 和 SSR 各有优缺点，同构渲染是一个折中的方案
- 将组件渲染为 HTML 字符串，也就是服务端渲染 SSR
  - 服务端不存在数据变更后的重新渲染，每次渲染时是应用的当前快照，无须调用 reactive 函数对 data 进行包装，也无须调用 shallowReactive 对 props 进行包装。也不需要调用 `beforeUpdate` 和 `updated` 钩子
  - 由于不存在真实的 DOM 挂载，无须调用 `beforeMount` 和 `mounted` 钩子
- 客户端激活 hydration
  - 将页面中的 DOM 元素与 vnode 对象之间建立联系
  - 为页面中的 DOM 元素添加事件绑定
- 如何编写同构代码，也就是既运行于客户端也运行于服务端的代码
  - 注意组件的生命周期，部分生命周期不会在服务端调用
  - 使用跨平台的 API
  - 某些内容可以选择仅在客户端渲染，通过 `ClientOnly` 组件来实现
